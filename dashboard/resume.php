 ...
 <tbody>
			...
			if ($row_AgentType['AgentType']=='2')
				{
					...
					$uploadloc	= "upload.php?lokasi=".$row_AgentType['AgentName'];
  ...
  ?>
    ...
    <tr>
      ...
      <td>
      <form id="form2" name="form1" method="post" action="<?php echo $uploadloc; ?>">
        <input name="a" type="hidden" id="a" value="<?php echo $row_AgentType['AgentName']; ?>" />
		<input name="b" type="hidden" id="b" value="<?php echo $row_AgentType['Sungai']; ?>" />
	  ...
	  <td>
		...
		//	$datelate 	= $datelate/(3600);
			...
			/*
			$timenow	= date("H:i:s");
			$timecur	= date("H:i:s",$row_received_date['SamplingTime']);
			$tg1		= round((strtotime($timenow)-strtotime($timecur))/3600)-1;
			*/
